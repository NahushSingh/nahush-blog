...
def register():
    user_from = RegisterUserForm()
    if user_from.validate_on_submit():

        if User.query.filter_by(email=user_from.email.data).first():
            flash("This email is already registered. Please Log In!")
            return redirect(url_for('login'))

        hash_and_salted_password = generate_password_hash(
            password=user_from.password.data,
            method="pbkdf2:sha256",
            salt_length=8
        )
        new_user = User(
            email=user_from.email.data,
            password=hash_and_salted_password,
            name=user_from.name.data
        )
    ...
    return render_template("register.html", form=user_from)
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if user and check_password_hash(pwhash=user.password, password=login_form.password.data):
            ...
            flash("Incorrect Password. Please provide valid password.")
            return redirect(url_for('login'))
...
def show_post(post_id):
    ...
    if user_comment.validate_on_submit():
        if not current_user.is_authenticated:
            ...
            return redirect(url_for('login'))
        new_comment = Comment(
            text=user_comment.comment_text.data,
